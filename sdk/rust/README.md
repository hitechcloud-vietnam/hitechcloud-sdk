# HiTechCloud hitechcloud-sdk — Rust SDK
...
```toml
[dependencies]
hitechcloud_sdk = "0.1.0"
```

## Quick Start

```rust
use hitechcloud_sdk::Client;

#[tokio::main]
async fn main() {
    let client = Client::new(Some("https://api.hitechcloud.vn"), Some("your-token".to_string()), Some(30), Some(3));
    let result = client.affiliate.get_summary(None).await.unwrap();
    println!("{:?}", result);
}
```
