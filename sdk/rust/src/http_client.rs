...
impl HttpClient {
    pub fn new(base_url: &str, token: Option<String>, timeout: u64, max_retries: u32) -> Self {
        let client = ReqwestClient::builder()
            .timeout(std::time::Duration::from_secs(timeout))
            .build()
            .expect("Failed to create HTTP client");
        Self { client, base_url: base_url.to_string(), token, max_retries }
    }
    ...
    fn add_headers(&self, req: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        ...
        match &self.token { Some(t) => req.header("Authorization", format!("Bearer {}", t)), None => req }
    }
}
