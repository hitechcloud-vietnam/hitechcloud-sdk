#!/usr/bin/env bash
# HiTechCloud hitechcloud-sdk HTTP Client

HITECHCLOUD_BASE_URL="${HITECHCLOUD_BASE_URL:-https://api.hitechcloud.vn}"
HITECHCLOUD_TOKEN="${HITECHCLOUD_TOKEN:-}"
HITECHCLOUD_TIMEOUT="${HITECHCLOUD_TIMEOUT:-30}"

_hc_headers() {
    local headers=(-H "Content-Type: application/json" -H "Accept: application/json")
    [ -n "$HITECHCLOUD_TOKEN" ] && headers+=(-H "Authorization: Bearer $HITECHCLOUD_TOKEN")
    echo "${headers[@]}"
}
