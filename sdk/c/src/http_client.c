...
hc_http_client_t *hc_http_client_new(const char *base_url, const char *token, long timeout, int max_retries) {
...
static struct curl_slist *make_headers(hc_http_client_t *c) {
    ...
    if (c->token) { char auth_hdr[512]; snprintf(auth_hdr, sizeof(auth_hdr), "Authorization: Bearer %s", c->token); h = curl_slist_append(h, auth_hdr); }
    return h;
}
